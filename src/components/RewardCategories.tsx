    ...
    <div className="bg-white/70 backdrop-blur-md border border-purple-100 shadow-lg rounded-2xl overflow-hidden">
      ...
      <div className="p-4 sm:p-6 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        {REWARDS.map(({ icon: Icon, label, title, iconBg, cardBg, borderColor, glowColor, ringColor, pulseColor, titleColor }, i) => (
          <div
            key={label}
            className={`group relative overflow-hidden ${cardBg} backdrop-blur-xl border-2 ${borderColor} rounded-2xl px-3 py-5 sm:px-4 sm:py-7 flex flex-col items-center text-center transition-all duration-500 hover:scale-[1.05] ${glowColor} cursor-default`}
            style={{ animation: `fadeSlideUp 0.5s ease-out ${i * 120}ms both` }}
          >
            {/* Animated shimmer overlay */}
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 bg-[linear-gradient(110deg,transparent_20%,rgba(255,255,255,0.5)_50%,transparent_80%)] bg-[length:250%_100%] group-hover:animate-[shimmer_2s_ease-in-out]" />
